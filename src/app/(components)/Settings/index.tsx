  ...
  return (
    <div>
      ...
      <div className={`fixed flex items-center justify-center bg-[#0a0c1c80] w-screen h-screen inset-0 duration-300 ${modalOpened ? 'z-20 opacity-100' : 'z-[-1] opacity-0'}`}>
        <div ref={modalRef} className="bg-white w-[90%] md:w-[740px] rounded-[25px] pt-[34px] pb-[59px] relative">
          {/* Title */}
          <header className="flex px-[40px] items-center justify-between">
            <h2 className="text-dark text-[20px] md:text-[28px] font-bold">Settings</h2>
          ...
          <div className="px-[40px]">
            {/* Subtitle */}
            <h3 className="text-dark text-center md:text-left text-[11px] md:text-[13px] font-bold tracking-[4.231px] md:tracking-[5px] uppercase mb-[24px]">Time (minutes)</h3>
            <div className="flex flex-col gap-[24px]">
              {/* Inputs */}
              <div className="grid md:grid-cols-3 gap-[20px]">
                <div className="flex justify-between items-center md:items-start md:flex-col gap-[8px]">
                    <label htmlFor="pomodoro" className="text-[12px] text-dark opacity-40 font-bold">pomodoro</label>
                    <input value={localPomodoroTime === 0 ? '' : localPomodoroTime} onChange={(e) => handleTime(e, setLocalPomodoroTime)} className="input-number w-[50%] md:w-full outline-none bg-[#EFF1FA] text-[14px] text-dark font-bold h-[48px] py-[16px] px-[20px] rounded-[10px]" type="number" name="pomodoro" id="pomodoro" placeholder="0" maxLength={2}  max={60} />
                </div>
                <div className="flex justify-between items-center md:items-start md:flex-col gap-[8px]">
                    <label htmlFor="short_break" className="text-[12px] text-dark opacity-40 font-bold">short break</label>
                    <input value={localShortBreakTime === 0 ? '' : localShortBreakTime} onChange={(e) => handleTime(e, setLocalShortBreakTime)} className="input-number w-[50%] md:w-full outline-none bg-[#EFF1FA] text-[14px] text-dark font-bold h-[48px] py-[16px] px-[20px] rounded-[10px]" type="number" name="short_break" id="short_break" placeholder="0" maxLength={2}  max={60} />
                </div>
                <div className="flex justify-between items-center md:items-start md:flex-col gap-[8px]">
                    <label htmlFor="long_break" className="text-[12px] text-dark opacity-40 font-bold">long break</label>
                    <input value={localLongBreakTime === 0 ? '' : localLongBreakTime} onChange={(e) => handleTime(e, setLocalLongBreakTime)} className="input-number w-[50%] md:w-full outline-none bg-[#EFF1FA] text-[14px] text-dark font-bold h-[48px] py-[16px] px-[20px] rounded-[10px]" type="number" name="long_break" id="long_break" placeholder="0" maxLength={2}  max={60} />
                </div>
              </div>
              {/* Wrapper */}
              <div className="bg-[#E3E1E1] h-[1px] w-full"></div>
              {/* Font */}
              <div className="flex flex-col gap-[18px] md:gap-0 md:flex-row justify-between items-center">
                <h3 className="text-dark text-[11px] md:text-[13px] font-bold tracking-[4.231px] md:tracking-[5px] uppercase">Font</h3>
              ...
              {/* Color */}
              <div className="flex  flex-col gap-[18px] md:gap-0 md:flex-row justify-between items-center">
                <h3 className="text-dark text-[11px] md:text-[13px] font-bold tracking-[4.231px] md:tracking-[5px] uppercase">Color</h3>
                <div className="flex gap-[16px]">
